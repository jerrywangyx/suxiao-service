...
// 按新的顺序定义工具
const toolsOrder = [
  // 第1组：今日热榜后面紧跟视频去水印、在线抠图、翻译、短链生成
  {
    name: '今日热榜',
    icon: 'fas fa-fire',
...
if (match) {
  ...
  console.log(`已调整工具顺序，共 ${toolsOrder.length} 个工具`);
  console.log('前5个工具：今日热榜、视频去水印、在线抠图、DeepL翻译、短链生成');
  console.log('最后2个工具：微信文件助手、临时邮箱');
} else {
  console.log('未找到工具直达区域');
}

fs.writeFileSync(filePath, content, 'utf8');
console.log('工具顺序调整完成！');
